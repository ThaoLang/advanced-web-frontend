...
const Pagination = (props: PaginationBarProp) => {
    ...
    if (props.current > 1) {
        items.push(
            <li key={props.current} onClick={() => { props.onChange(props.current - 1); createPageURL(props.current-1); }}>
    ...
    for (let page = minPage; page <= maxPage; page++) {
        items.push(
            page === props.current ? (
                <li key={props.current} onClick={() => { props.onChange(page); createPageURL(page); }}>
                    <p
                        aria-current="page"
