...
export default function AuthForm(props: AuthFormProps) {
    ...
    const handleLogin = async () => {
        ...
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_PREFIX}auth/admin/login`, {
                email: email,
                password: password,
            });
            if (response.status === 201) {
                ...
                localStorage.setItem("admin", JSON.stringify(cur_user));
            }
            router.push("/admin");
        } catch (error: any) {
            const errorMessage =
    ...
    const handleSignup = async () => {
        ...
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_PREFIX}auth/admin/register`, {
                email: email,
                password: password,
                username: username,
                role: 'admin',
        ...
        <div className="w-full border rounded-xl p-4 m-8 mx-auto my-auto bg-white xl:w-1/2">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
                ...
                <div>
                    {isSignupOpeneded && (
                        <div className="mb-4">
                            <label className="mb-2.5 block font-medium text-black dark:text-white">{t("username")}</label>
                    ...
                    <div className="mb-4">
                        <label className="mb-2.5 block font-medium text-black dark:text-white">{t("email")}</label>
                        <div className="relative">
                            <input type="email"
                                placeholder={t("email")}
                                className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                    ...
                    <div className="mb-4">
                        <label className="mb-2.5 block font-medium text-black dark:text-white">{t("password")}</label>
                        <div className="relative">
                            ...
                            <button className="absolute right-5 top-5 text-2xl"
                    ...
                    {isSignupOpeneded && (
                        <>
                            <div className="mb-5">
                                <button type="submit"
                                    className="w-full cursor-pointer rounded-lg btn btn-info bg-info p-4 font-medium text-white transition hover:bg-opacity-90"
                    ...
                    {/* 
                                <div className="mt-6 text-center">
                                    <p className="font-medium">
                                        ...
                                        <a href="signin.html" className="text-primary"> Sign in</a>
                                    </p>
                                </div> */}
