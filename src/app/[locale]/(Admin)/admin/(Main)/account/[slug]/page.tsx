...
async function getClassDetailsByUserId(user_id: string) {
  const matchingClassDetails: Array<ClassListType> =
    templateClassDetails.filter(
...
export default function Page({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    const loadClassInfo = async () => {
      ...
      context.classDetails = await getClassDetailsByUserId(
        context.account?.id ?? ""
      );
      setStudentId(
        context.classDetails.find((items) => items.student_id !== "")
          ?.student_id ?? ""
      );
            ...
            <div className="mt-10 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
              ...
              <div className="p-7">
                {userClassList?.map((items, index) => (
                  <div
                    key={index}
                    className="mb-5 card card-side bg-base-100 shadow-lg"
                  >
                    ...
                    <div className="card-body">
                      ...
                      <p className="">
                        Role: {context.classDetails?.at(0)?.role}
                      </p>
                    </div>
                  </div>
                ))}
